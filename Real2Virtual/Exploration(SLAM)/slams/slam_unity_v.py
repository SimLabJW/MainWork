...
class RealtimeSLAM:
    ...
    def _make_MRTSP_frontier(self, robot_x, robot_y, robot_theta):
        ...
        e_time = time.time()
        self.frontier_exploration_time.append(e_time - s_time)

        self.frontier_number.append(len(res))
    ...
    def _save_metrics_json(self):
        """
        수집한 지표들을 JSON으로 저장한다.
        - frontier_number: [각 PROCESS에서 검출된 프론티어 수]
        - frontier_exploration_time: [각 PROCESS에서 프론티어 선택/계산에 걸린 시간(s)]
        - path_length_cells: [각 PROCESS에서 A* path_len(셀 개수)]
        - path_length_m: [각 PROCESS에서 경로 길이(미터)]
        - exploration_number: 전체 탐색 횟수(= PROCESS 카운트)
        - exploration_success: 경계기반 성공도 (1 - free↔unknown 경계율)
        """
        try:
            export_dir = self._ensure_dir("./exports")
            ts = self._timestamp()
